...
with dai.Device(pm.pipeline.getOpenVINOVersion(), deviceInfo, usb2Mode=conf.args.usbSpeed == "usb2") as device:
    ...
    if conf.useCamera or conf.args.sync:
        ...
        if conf.rgbCameraEnabled:
            pm.createColorCam(nnManager.inputSize if conf.useNN else conf.previewSize, rgbRes, conf.args.rgbFps,
                              orientation=conf.args.cameraOrientation.get(Previews.color.name),
                              fullFov=not conf.args.disableFullFovNn,
                              xout=Previews.color.name in conf.args.show and (conf.getModelSource() != "color" or not conf.args.sync)
                              )
        ...
        if conf.useDepth:
            pm.createDepth(
                ...
                useRectifiedRight=Previews.rectifiedRight.name in conf.args.show and (
                ...
                if sbbOut is not None:
                    ...
                    for depthFrame in depthFrames:
                        ...
                        for roiData in sbbRois:
                            ...
                            cv2.rectangle(depthFrame, (int(topLeft.x), int(topLeft.y)), (int(bottomRight.x), int(bottomRight.y)),
                                          nnManager._bboxColors[0], 2)
            else:
